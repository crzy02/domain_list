...
REPO = os.environ.get("GITHUB_REPOSITORY", "crzy02/domain_list")
...
def read_sources():
    ...
    for ln in open(SOURCES, encoding="utf-8").read().splitlines():
        ...
        # Пятое поле — фильтр наборов.
        # Положительные имена образуют whitelist, имена с `-` — blacklist.
        # Например: `telegram,youtube,-ads`.
        raw_filter = parts[4] if len(parts) > 4 and parts[4] else ""
        include = set()
        exclude = set()
        for item in re.split(r"[,\s]+", raw_filter):
            ...
            if item.startswith("-") and len(item) > 1:
                exclude.add(item[1:].lower())
            else:
                include.add(item.lower())
        out.append((repo, prefix, title, kinds, include, exclude))
    return out


def upstream_lists(repo, prefix, title, kinds="auto", include=None, exclude=None):
    ...
    for aname in sorted(assets):
        ...
        base = aname[:-4]
        base_key = base.lower()
        if include and base_key not in include:
            continue
        ...
        common = {
            ...
            "source": repo, "source_name": title, "default_on": True,
        }
...
def main():
    ...
    cats, doms = own_lists()
    say("чужие релизы:")
    for repo, prefix, title, kinds, include, exclude in read_sources():
        try:
            c, d = upstream_lists(repo, prefix, title, kinds, include, exclude)
        except Exception as e:  # noqa: BLE001
